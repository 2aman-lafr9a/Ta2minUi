...
const SigninForm = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // using signin interface to define the type of the event target
    setFormData({
      ...formData,
  ...
  //       .then((data) => {
  //         if (data.success) {
  //           toast.success(`Welcome${ data.userName}`) // Displays a success message
  //           window.location.href = "/admin/dashboard"
  //         } else {
  //          toast.error(data.message) // Displays an error message
  //         }
  //       })
  //       .catch((err) => {
  //         toast.error(err.message) // Displays an error message
  //       })
    ...
    <svg viewBox="0 0 866 866" xmlns="http://www.w3.org/2000/svg">
      ...
      <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 164.83 151.5">
        ...
        <path className="path-4" d="M52.58,29.89c-2.15-.36-3.78-.54-5.39-.9-2.83-.64-4.92.1-7,2.32A64.1,64.1,0,0,0,26.09,54.64c-2.64,7.92-2.62,7.84,5.15,10.87,1.76.69,2.73.45,3.93-1C39.79,59,44.54,53.65,49.22,48.2a4.2,4.2,0,0,0,1.13-2c.8-5.32,1.49-10.68,2.24-16.34Z"
          fill="url(#gradient)" />
        <path className="path-5" fill="url(#gradient)" d="M23,68.13c0,2.51,0,4.7,0,6.87a60.49,60.49,0,0,0,9.75,32.15c1.37,2.13,6.4,3,7,1.2,1.55-5,2.68-10.2,3.82-15.34.13-.58-.58-1.38-.94-2.06-2.51-4.77-5.47-9.38-7.45-14.37C32.94,71,28.22,69.84,23,68.13Z" />
        <path className="path-6" fill="url(#gradient)" d="M83.91,12.86c-.32.36-.66.71-1,1.07.9,1.13,1.57,2.62,2.73,3.33,4.71,2.84,9.56,5.48,14.39,8.1a9.29,9.29,0,0,0,3.13.83c5.45.69,10.89,1.38,16.35,1.94a10.41,10.41,0,0,0,3.07-.71c-11.48-9.9-24.26-14.61-38.71-14.56Z"
        />
        <path className="path-7" fill="url(#gradient)" d="M66.28,132.51c13.36,3.78,25.62,3.5,38-.90C91.68,129.59,79.36,128,66.28,132.51Z" />
        <path className="path-8" fill="url(#gradient)" d="M127.2,30.66l-1.27.37a18.58,18.58,0,0,0,1,3.08c3,5.52,6.21,10.89,8.89,16.54,1.34,2.83,3.41,3.82,6.49,4.9a60.38,60.38,0,0,0-15.12-24.9Z" />
        <path className="bb-9" fill="url(#gradient)" d="M117.35,125c5.58-2.32,16.9-13.84,18.10-19.20-2.41,1.46-5.18,2.36-6.78,4.23-4.21,5-7.89,10.37-11.32,15Z" />
      </svg>
    </svg>
  </div>
  </div>
